...
function search(){
  let pressed_cell = this
  let isFree = pressed_cell.innerText.length
  // let free_cell = pressed_cell.innerText
  // if(free_cell = free_cell.innerHTML('<div class="cell" id="cell">X</div>')){
  //   return isFree=false, console.log(isFree)
  // // }
  // console.log(free_cell)
  if(isXorO && isFree<1){
    pressed_cell.innerText= 'X';
    isWin()
    return isXorO=false
  }
  if(isXorO==false && isFree<1){
